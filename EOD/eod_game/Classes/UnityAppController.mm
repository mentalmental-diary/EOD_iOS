...
extern "C" {
    void _sendMessageToiOS(const char* message) {
        ...
        NSLog(@"Received message from Unity: %@", messageStr);
        // 추가적인 데이터 처리 로직 작성 가능
        
        [[NSNotificationCenter defaultCenter] postNotificationName:@"UnityMessage" object:nil userInfo:@{@"message": messageStr}];
    }
}

extern "C"
{
    void SendMessageToUnity(const char* gameObject, const char* methodName, const char* message)
    {
